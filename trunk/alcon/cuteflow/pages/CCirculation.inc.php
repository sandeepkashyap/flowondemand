...
class CCirculation
{
	...
	*/
	function addCirculationForm($strCirculationName, $nMailinglistID, $nSenderID, $SuccessMail, $SuccessArchive, $SuccessDelete)
	{
		$dateSending = mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y"));
	
		$nEndAction = 0;
		
		if ($SuccessMail == 'on') $nEndAction += 1;
		if ($SuccessArchive == 'on') $nEndAction += 2;
		if ($SuccessDelete == 'on') $nEndAction += 4;
		
		$strQuery 	= "INSERT INTO cf_circulationform values (null, '$nSenderID', '$strCirculationName', '$nMailinglistID', 0, '$nEndAction', 0)";
		$nResult 	= @mysql_query($strQuery);
	...
	*/
	function getUsers()
	{
		$strQuery = "SELECT * FROM cf_user ORDER BY strUserId;";
		$nResult = @mysql_query($strQuery);
		if ($nResult)
		{
			while (	$arrRow = mysql_fetch_array($nResult, MYSQL_ASSOC))
	...
	*/
	function getUsername($nUserID)
	{
		...
		return $arrRows[0]['strUserId']." (".$arrRows[0]['strFirstName'].", ". $arrRows[0]['strLastName'].")";
	}
	...
	*/
	function getCirculationOverview($start, $sortby, $sortDirection, $archivemode, $nShowRows, $bFilterOn, $FILTER_Name = '', $FILTER_Sender = false, $FILTER_Mailinglist = false, $FILTER_Station = false, $FILTER_DAYS_IN_PROGRESS_START = '', $FILTER_DAYS_IN_PROGRESS_END = '', $FILTER_DATE_START = '', $FILTER_DATE_END = '', $FILTER_TEMPLATE = false, $FILTER_CUSTOM = false)
	{
		...
		if ($FILTER_CUSTOM)
		{
			...
			{
				$strFilterCustomQuery .= " AND nInputFieldId = '$nInputfieldId' AND strFieldValue $strOperator '$strValue'";
			}
		}
