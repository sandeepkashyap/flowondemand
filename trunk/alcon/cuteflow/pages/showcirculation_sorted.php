...
include	('../language_files/language.inc.php');
	...
	{	// extended filter has been activated
		...
		{
			// TODO
			if (($FILTER_CUSTOM != false) && (sizeof($FILTER_CUSTOM) > 1))
			{
				...
				for ($nIndex = 1; $nIndex < $nMax; $nIndex++)
				{
					...
					if ($strValue != '')
					{
						$arrResults = array();
						$nMyCounter = 0;
						$nMax = sizeof($arrCirculationOverview);
						for($nIndex2 = 0; $nIndex2 < $nMax; $nIndex2++)
						{
    ...
    <table width="90%">
		<tr>
			<td align="left">
				...
				if (!$_REQUEST['bFilterOn'])
				{
					$nAmountOfCirculations = $objCirculation->getAmountOfAllCirculations($archivemode);
				}
				else
				{
