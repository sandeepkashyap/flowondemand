...
function checkDB($username, $password)
{
	...
	$query 			= "select * from cf_user where strUserId = '$userID'";
	...
	if ($nResult)
	{
		...
		{		
			...
			if ($arrRow)
			{
				...
				$query .=	"WHERE strUserId	= '".$LDAP['uname']."'";
				...
				if ($nResult) //The update was processed
				{
					...
					$strQuery 	= "SELECT * FROM cf_user WHERE strUserId = '".$LDAP['uname']."' LIMIT 1;";
					...
					$strQuery = "UPDATE cf_user_index SET `index` = '$strIndex' WHERE user_id = '".$arrResult['nID']."' LIMIT 1";
		...
		{			
			...
			// write the index String
			$strQuery = "INSERT INTO cf_user_index values (	'".$arrResult['nID']."',
															'$strIndex')";
			$nResult = @mysql_query($strQuery, $connection);
