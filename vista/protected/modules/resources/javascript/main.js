      ...
      if (!initial_text || initial_text.length < 1) {
        ...
        dialog_body = dialog_body +
          '<div class="buttonHolder">'+
            '<button accesskey="s" type="submit"><span>'+App.lang('Insert link')+'</span></button>'+
          '</div>'+
        '</div>'+
        '</form>'+
      '</div>';
      ...
      '<div id="editor_clean_text_dialog_container">' +
        '<form method="post" action="#" class="uniForm">' +
          '<div class="blockLabels">' +
            ...
            '</div>' +
            '<div class="buttonHolder">'+
              '<button accesskey="s" type="submit"><span>'+App.lang('Insert Cleaned Text')+'</span></button>'+
            '</div>'+
          '</div>' +
        '</form>' +
      '</div>';
...
/*  if (!App.data.copyright_removed && $('#footer #powered_by a[href=htp://www.activecollab.com]').length == 0) {
    if ($('#footer').length == 0) {
      $('body').append('<div id="footer"></div>');
    }*/ // if
/*    $('#footer').prepend('<p id="powered_by"><a href="htp://www.activecollab.com" target="_blank"><img src="' + App.data.assets_url + '/images/acpowered.gif" alt="ActiveCollab" /></a></p>').css('display', 'block').css('visibility','visible').css('position', 'static');*/
