...
App.system.controllers.dashboard = {
  ...
  trash : function() {
    $(document).ready(function() {
      $('#option_empty_trash').click(function() {
        return confirm(App.lang('Are you sure that you wish to permanently delete all items in the trash?'));
      });
    ...
    $(document).ready(function() {
      ...
      var enable_disable_smtp_settings = function() {
        if($('#mailingType').val() == 'smtp') {
          $('#smtpSettings').show();
          $('#smtpSettings input').attr('disabled', '');
          $('#mailingSecurity').attr('disabled', '');
        } else {
          $('#smtpSettings').hide();
          $('#smtpSettings input').attr('disabled', 'disabled');
...
/**
 * Project tools behavior
 */
App.system.controllers.system_tools = {
  
  /**
   * Scheduled tasks behavior
   */
  scheduled_tasks : function() {
    $(document).ready(function() {
      $('#scheduled_tasks td.event_url a').click(function() {
        prompt(App.lang('Event URL'), $(this).attr('href'));
        return false;
      });
    });
  }
...
App.system.QuickAdd = function() {
  ...
  return {
    ...
    init_step_1 : function(skip_initial) {
      ...
      if (!step_1_initialized) {
        ...
        step_1.find('.continue').click(function () {
          App.system.QuickAdd.init_step_2(project_chooser.find('input[checked=true]').val(),object_chooser.find('input[checked=true]').val(), object_chooser.find('input[checked=true]').attr('quick_add_url'), project_chooser.find('input[checked=true]').parent().text());
          return false;
        });
        ...
        project_chooser.find('input').click(function () {
          ...
          for (var counter = 0; counter < project_objects.length; counter++) {
            list_elements += '<label for="type_' + project_objects[counter] + '"><input type="radio" name="object_type" value="' + project_objects[counter] + '" class="input_radio" id="type_' + project_objects[counter] + '" quick_add_url="' + App.data.quick_add_urls[project_objects[counter]] + '" />' + ucfirst(App.lang(project_objects[counter])) + '</label>';
          } // for
          object_chooser.html(list_elements);
     ...
     * @return null
     */
    init_step_2 : function (project_id, object_type, object_add_url, project_name) {
      step_1.hide();
      step_2.show();
      ...
      $.ajax({
        ...
        success : function (response) {
          ...
          if (App.ModalDialog.isOpen) {
            App.ModalDialog.setTitle(App.lang('Quick Add :object_type in :project_name', {'object_type' : ucfirst(object_type), 'project_name' : project_name}));
          } // if
...
App.widgets.IconPicker = function() {
  ...
  return {
     ...
     *
     */
    init : function (icon_picker, icon_to_update) {
      ...
      icon_container.click(function () {
        App.ModalDialog.show('icon_picker', App.lang('Change Icon'), $('<p><img src="' + App.data.assets_url + '/images/indicator.gif" alt="" /> ' + App.lang('Loading...') + '</p>').load(picker_url, function () {
